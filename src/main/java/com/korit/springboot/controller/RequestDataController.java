...
//    @CrossOrigin("http://localhost:5173")
    @GetMapping("/req/data1") //http://localhost:8080/req/data1?a=김백년&b=4
    ...
    // GET 요청 파라미터 받는 방법 2(스프링부트)
    @GetMapping("/req/data2") //리퀘스트파람이 형변환도 해줌
    ...
    // GET 요청 파라미터 받는 방법 3(스프링부트) - 변수명과 파라미터 명이 일치하면 @RequestParam의 이름을 생략할 수 있다.
    ...
    // GET 요청 파라미터 받는 방법 4(스프링부트) - 해당 파라미터를 필수 항목으로 설정할 수 있다. (기본 값 True)
    // 정수 자료형의 파라미터가 필수가 아닐 때 Integer 자료형을 사용해야 한다.(int는 null값 못받아서 Integer 사용)
    ...
    // GET 요청 파라미터 받는 방법 5(스프링부트) - 파라미터가 많아졌을 때 정리하는 방법
    ...
    // GET 요청 파라미터 받는 방법 6(스프링부트) - 파라미터가 많아졌을 때 DTO로 정리하는 방법
    ...
    // GET 요청 파라미터 받는 방법 7(스프링부트) - 주소에서 값 가져오기
    ...
    public ResponseEntity<Map<String, String>> reqGet7(
        ...
        System.out.println("data7: " + dto.getAddress());
        System.out.println("data7: " + dto.getPhone());
        return ResponseEntity.ok().build();
    }
}
